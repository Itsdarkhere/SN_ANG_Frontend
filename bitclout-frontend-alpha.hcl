job "supernovas-frontend-alpha" {
  datacenters = ["supernovas-fin"]
  type = "service"


  group "supernovas-bclt-front-alpha" {
    count = 4
    ...
    task "supernovas-bclt-frontend-alpha" {
      driver = "docker"
      config {
...
:{{ env "NOMAD_PORT_http" }} {
    ...
    header Access-Control-Allow-Origin "https://testnovas.app"
...
EOF
      ...
      service {
        name = "supernovas-frontend-alpha"
        port = "http"
