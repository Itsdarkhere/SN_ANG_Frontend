...
import { BackendApiService, NFTCollectionResponse, ProfileEntryResponse } from "../backend-api.service";
    ...
    /*this.backendApi
      ...
      .subscribe(
        (res: any) => {
          ...
          THIS COMBO REMOVES DUPLICATES FROM DROP
          console.log(this.nftCollections);
          for (let x of this.nftCollections) {
            this.list2.push(x.PostEntryResponse.PostHashHex);
            this.list3.push(x.PostEntryResponse.PostHashHex);
          }
          console.log(this.nftCollections.length);
          this.list2 = uniqBy(this.list2, (nftCollection) => nftCollection);
          let list4 = [];
          for (let x of this.list2) {
            let z = 0;
            for (let y of this.list3) {
              if (x === y) {
                if (z > 0) {
                  list4.push(y);
                }
        ...
        (res) => {
          ...
          for (let x of this.adminPosts) {
            if (this.PKList.includes(x.PosterPublicKeyBase58Check) && x.IsNFT) {
              if (!this.list2.includes(x.PostHashHex)) {
                console.log(x.PostHashHex + " " + x.TimestampNanos);
                this.addAnNFT(x.PostHashHex);
              }
            }
          }
          if (this.adminPosts && bool) {
            setTimeout(
              function () {
