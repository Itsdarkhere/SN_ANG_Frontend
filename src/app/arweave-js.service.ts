...
export class ArweaveJsService {
    ...
    return from(file.arrayBuffer()
      ...
      .then((data) => {
        if (data.uploader.totalChunks > 1) {
          return [...Array(data.uploader.totalChunks - 1).keys()].reduce(
            (acc, index) => acc.then(() => {
              console.log(`${data.uploader.pctComplete}% complete, ${data.uploader.uploadedChunks + 2}/${data.uploader.totalChunks}`);
  ...
  ConfirmTransaction(txnId: string): Observable<boolean> {
    ...
    return from(
      [...Array(100).keys()].reduce((acc, index) => {

          return acc.then((lastStatus) => {
            if ((lastStatus.confirmed != null) && 
              (lastStatus.confirmed.number_of_confirmations > 1)) {
      ...
      .then((lastStatus) => ((lastStatus.confirmed != null) && 
              (lastStatus.confirmed.number_of_confirmations > 1)))
    );
  }
