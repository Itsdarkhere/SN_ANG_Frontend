...
export class ArweaveJsService {
  ...
  startUpload(file: File): Observable<string> {
    return from(file.arrayBuffer()
      ...
      .then((data) => {
        if (data.uploader.totalChunks > 1) {
          return [...Array(data.uploader.totalChunks - 1).keys()].reduce(
            (acc, index) => acc.then(() => {
              console.log(`${data.uploader.pctComplete}% complete, ${data.uploader.uploadedChunks}/${data.uploader.totalChunks}`);    
      ...
      .then((data) => {
        let url = "https://arweave.net/" + data.txn.id;
  ...
  getBalance() {
    this.client.wallets.getBalance(this.publicKey).then((balance) => {
      let winston = balance;
      let ar = this.client.ar.winstonToAr(balance);
